...
<body>
	...
	<table id="snsWriteArea" style="left: 0px;">
		<tr>
					...
					<table id="snsSearchTable">
						<tr>
							<td id="sstTd1">
								...
								autocomplete="off" placeholder="찾"></td>
			...
			<td align="center" rowspan="2"><img src="resources/img/menu.png"
			...
			<tr>
				<td id="smReplyArea">
					<c:forEach var="sr" items="${sm.reply }">
						<c:choose>
							<c:when test="${sr.dsr_owner == sessionScope.loginMember.dm_id }">
								<span ondblclick="snsReplyDelete('${sr.dsr_no}');" class="smReplyID" style="color:${sm.ds_color}; cursor:pointer;" >${sr.dsr_owner }</span>	
							</c:when>
							<c:otherwise>
								<span class="smReplyID" style="color:${sm.ds_color}">${sr.dsr_owner }</span>
							</c:otherwise>
						</c:choose>
						${sr.dsr_txt }(<fmt:formatDate value="${sr.dsr_when }" type="both" dateStyle="short" timeStyle="short"/>)<br>
					</c:forEach>
					...
					<form action="sns.reply.write" method="post" 
						...
						<span class="smReplyID" style="color:${sm.ds_color}">${sessionScope.loginMember.dm_id }</span>
						<input name="dsr_txt" style="border-bottom:${sm.ds_color} solid 2px;" maxlength="100" placeholder="댓" autocomplete="off">
			...
			<tr>
				<td colspan="2" align="left">
					<c:if test="${sm.dm_id == sessionScope.loginMember.dm_id }">
						<button onclick="snsMsgUpdate(${sm.ds_no}, '${sm.ds_txt }');" style="color: ${sm.ds_color};">수정</button>
						<button onclick="snsMsgDelete(${sm.ds_no});" style="color: ${sm.ds_color};">삭제</button>
					</c:if>
				</td>
			</tr>
		</table>
